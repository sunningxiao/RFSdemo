...
class RFSKitRPCServer(SimpleXMLRPCServer, LightDMAMixin):
    ...
    def rpc_set(self, name, value=0, channel=1, execute=True):
        ...
        match name:
            case 'Waveform':
                result = self.param_waveform_conf(channel, value)
            case 'GenWave':
                result = self.param_genwave_conf(channel, value)
            case 'GenWaveIQ':
                result = self.param_genwaveiq_conf(channel, value)
            case 'Delay':
                result = self.param_delay_conf(channel, value)
            case 'LinSpace':
                result = self.param_linspace_conf(channel, value)
            case 'Output':
                result = self.param_output(channel, value)

            case 'ADrate':
                result = self.param_adrate_conf(value)
            case 'DArate':
                result = self.param_darate_conf(value)
            case 'Shot':
                print(f'shots: {value}')
    ...
    def param_darate_conf(self, value):
        """
        配置da采样率
    ...
    def param_adrate_conf(self, value):
        """
        配置ad采样率
    ...
    def param_output(self, channel, value):
        """
        配置各个dac使能
    ...
    def param_linspace_conf(self, channel, value):
        """
        配置计算下发波形用的waveform
    ...
    def param_delay_conf(self, channel, value):
        """
        配置dac延迟，单位s
    ...
    def param_genwaveiq_conf(self, channel, value):
        """
        iq分别配置的waveform对象
    ...
    def param_genwave_conf(self, channel, value):
        """
        接收waveform对象, 计算后向下配置
    ...
    def param_waveform_conf(self, channel, value):
        """
        直接接收并向下配置成品波形
